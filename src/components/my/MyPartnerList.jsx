...
function MyPartnerList() {
  ...
  const handleDismiss = async (id) => {
    ...
    try {
      ...
      toast(<ToastMessage>제휴가 목록에서 숨김 처리되었습니다.</ToastMessage>, {
          position: "top-center",
          autoClose: 3000,
    ...
    <>
      <div className={styles.listContainer}>
        <h2 className={styles.mainTitle}>
          <span className={styles.placeName}>{placeName || "사장"}</span>님과
          제휴 맺은 업장
        </h2>
              ...
              <div key={partner.partnershipId} className={styles.partnerCard}>
                <div className={styles.partnerInfo}>
                  <p className={styles.partnerName}>
                    {partner.partnerPlaceName}
                  </p>
                  <p className={styles.partnerAddress}>
                    {partner.partnerStoreAddress}
                  </p>
                </div>
